# VibeFrame
A fork of Kenya-DK's QuantFrame with a flatpak package distribution
...
### Installing a release `.flatpak` bundle (end users)

Each GitHub release ships a single-file `Quantframe-x86_64-<version>.flatpak`
bundle. Users do **not** need to build from source:

```bash
# One-time: make sure Flathub is configured so the GNOME 49 runtime can
# be pulled in automatically the first time you install Quantframe.
flatpak remote-add --if-not-exists --user flathub https://flathub.org/repo/flathub.flatpakrepo

# Install the downloaded bundle
flatpak install --user Quantframe-x86_64-1.6.20.flatpak

# Run
flatpak run dev.kenya.quantframe
```

The bundle pins its runtime dependency to
`https://dl.flathub.org/repo/flathub.flatpakrepo`, so `flatpak install`
will offer to pull the GNOME 49 runtime from Flathub if it isn't
already present on the system.

### Producing a release bundle (maintainers)

To build the `.flatpak` file that goes on a GitHub release:

```bash
# 1. Build and export to a local OSTree repo
flatpak-builder --user --force-clean --disable-rofiles-fuse \
--repo=flatpak-build/repo \
flatpak-build/builddir \
packaging/flatpak/dev.kenya.quantframe.yml

# 2. Pack the exported ref into a single-file bundle
...
The resulting `Quantframe-x86_64-<version>.flatpak` (~13 MB) is the file
to upload as a GitHub release asset.
...
Everything below this is the original repo README.md
